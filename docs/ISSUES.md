...
## Future Calls bypass the streaming/broadcast layer

**Priority:** P0

**Where:** `roundtable_server/lib/src/future_calls/machine_offline_future_call.dart`,
`roundtable_server/lib/src/future_calls/stalled_task_future_call.dart`.

Both future calls write directly via `Task.db.updateRow`/
`Machine.db.updateRow` instead of going through `TaskEndpoint`/
`MachineEndpoint`. Every state mutation made through those endpoints also
calls `session.messages.postMessage` on `_channelForTask`/
`_channelForAllTasks`/`_channelForMachine`, which is what wakes
`watchTask`/`watchAllTasks`/`watchAssignedTasks` subscribers. The future
calls skip that broadcast entirely.

**Why not fixed yet:** surfaced during this review, not yet triaged into
a specific change.

**Possible directions:**
- Route the future calls' writes through the same endpoint methods (or a
  shared internal helper that both the endpoint and the future call use)
  so the broadcast always happens alongside the DB write.
- At minimum, have each future call call `session.messages.postMessage`
  itself after the direct `updateRow`, mirroring the channel-naming
  helpers already used in `TaskEndpoint`/`MachineEndpoint`.

**User-facing impact today:** a dashboard subscribed via `watchAllTasks`
does not see a task flip to `failed` when the stalled-task or
machine-offline detector fires — it only reflects on next full reload.
Worse, the daemon (subscribed via `watchTask`) is never told its task was
force-failed, so it may keep running a subprocess the server has already
given up on. This directly undermines the "live status" feature the
product is built around, and is very likely to be hit while manually
testing stall/offline scenarios.

---

## Blocked project deletion silently reports as success

**Priority:** P0

**Where:** `roundtable_flutter/lib/cubits/project_list_cubit.dart`,
`ProjectListCubit.deleteProject`; `roundtable_flutter/lib/screens/project_detail_screen.dart`,
the delete button's `onPressed` handler.

`deleteProject` unconditionally calls `await fetchProjects()` after its
`try`/`catch` block, including after catching a `DeletionBlockedException`
and emitting `ProjectListError`. Because the refetch runs and completes
before the awaited `deleteProject()` call returns, by the time
`project_detail_screen.dart` checks `cubit.state is ProjectListError` the
state has already been overwritten by whatever `fetchProjects()` emitted
(typically back to `ProjectListLoaded`). The screen falls into its
"success" branch and calls `Navigator.of(context).pop()`.

**Why not fixed yet:** surfaced during this review.

**Possible directions:**
- Mirror `MachineListCubit`/`machines_screen.dart`'s pattern: use
  `BlocConsumer.listener` to react to the transient error state as it's
  emitted, instead of inspecting `cubit.state` after the whole async
  chain (including the refetch) has settled.
- Alternatively, have `deleteProject` return a `bool`/result type instead
  of relying on the caller re-reading post-refetch state.

**User-facing impact today:** deleting a project that's blocked by
`DeletionBlockedException` (non-terminal tasks) appears to succeed —
the dialog closes and the user is returned to the project list — even
though the project was not actually deleted, with no explanation given.

---

## Dead "Open PR" buttons in task detail

**Priority:** P0

**Where:** `roundtable_flutter/lib/screens/task_detail_screen.dart`, the
`_InfoRail` "Open PR" button (~line 279) and `_DiffReview`'s "View full
PR on GitHub" button (~line 594).

Both buttons are shown whenever `task.prUrl != null` — i.e. real PR data
is available — but both have `onPressed: () {}`. No `url_launcher`
dependency is wired up to actually open the link. Unlike the intentionally
disabled `docker` execution mode option in `add_agent_dialog.dart` (which
is marked "Coming soon"), there's no marker here indicating this is a
known stub rather than an oversight.

**Why not fixed yet:** surfaced during this review.

**Possible directions:**
- Add `url_launcher` and wire both buttons to open `task.prUrl`.
- Until then, disable the buttons (with a tooltip) rather than leaving
  them clickable and silently doing nothing.

**User-facing impact today:** both buttons look like live actions but do
nothing when clicked — in a "command center" UI this reads as broken
immediately, especially since it's the terminal step of the reviewed
workflow (going to look at the actual PR).
